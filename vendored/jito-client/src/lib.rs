...
use jito_protos::{
    auth::{Role, auth_service_client::AuthServiceClient},
    bundle::{
        Accepted, Bundle, BundleResult, InternalError, SimulationFailure, StateAuctionBidRejected,
        WinningBatchBidRejected, bundle_result::Result as BundleResultType, rejected::Reason,
    },
...
pub async fn setup_bundle_results_stream<T>(
    searcher_client: &mut SearcherServiceClient<T>,
) -> BlockEngineConnectionResult<Streaming<BundleResult>>
where
...
{
    let bundle_results_stream = searcher_client
        .subscribe_bundle_results(SubscribeBundleResultsRequest {})
        .await?
        .into_inner();

    Ok(bundle_results_stream)
}
