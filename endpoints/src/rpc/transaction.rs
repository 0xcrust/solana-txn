use crate::defaults::GET_TXN_CONCURRENCY;
use std::str::FromStr;
use std::sync::Arc;
...
/// Get the most recent transaction for a given wallet.
...
pub async fn get_signatures_for_address(
    ...
    loop {
        ...
        // If `before` is specified then it is not included in the result.
        // If `before` is not specified then it defaults to the most recent signature, and includes that in the response.
...
pub async fn get_transactions_for_signatures(
    ...
    let concurrency = concurrency.unwrap_or(GET_TXN_CONCURRENCY);
